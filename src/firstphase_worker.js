...
// This file holds first phase specific functions. 
	...
	"peptides":[{"id":44982,"sequence":"NHFNANCSFWNYSER",varmod:{id, mass,"residue sequence"}},...]
	"fixedMods":[{"mass":57.021464,"residue":"C"}],"fragTol":0.01,"fragTolUnit":"da"}
	"varmod" is optional and occurs in pass 2 type workunits
	*/
	
	//First phase search only deals with fixed mods - so only one set of ion fragments to check
	
	function doFirstPhaseSearch(myWorkUnit)
	{
		var allPeptideScores=[];							//array of scoreObjects {id:0, score:0} one per peptide matched 
		var resultObject= { job:myWorkUnit.job, precursor:myWorkUnit.precursor, peptides:[] };								//return object
		var scoreObj = {id:0, ionsMatched:0, score: 0};
		var numRes = MAX_NUMBER_SCORES_RETURNED;			//number of results to return
				 
		for (var p = 0; p < myWorkUnit.peptides.length; p++){
			var ionset = fixedOnlyFragment(myWorkUnit.peptides[p]);				//ionset = {modpos[], bIons[],yIons[]} //we don't need modpos in this phase
			ionset = matchSpectraWithIonSet(myWorkUnit.fragments, ionset);		//fills out match and intensity in bIons[] and yIons[];
			scoreObj = scoreIonset(ionset);											//fills out score and ionsMatched in {ionsMatched:0, score: 0};
		...
		console.log("job = "+resultObject.job+", pre = "+resultObject.precursor+", peps tested = "+myWorkUnit.peptides.length+", peps returned = "+resultObject.peptides.length);
		postMessage(retString);
	}
	
	
	//Phase one fragmentation. - only fixed modifications are taken into account.
	//returns a single phase one ionSet object {bIons:[{mass,match,intensity,deltaM},..],yIons:[{mass,match,intensity,deltaM},..]}
	function fixedOnlyFragment(myPeptide)
	{
		var ionSet = new Ionset();	//{bIons:[],yIons:[]};		//bIons = [{mass, match, intensity}]		
