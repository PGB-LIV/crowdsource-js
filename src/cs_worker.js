...
function scoreIonset(ionSet)
{
	...
	bscore = (ION_MATCH_SCALE*bcount)/ionSet.bIons.length;		// 0 - 25%
																// for
																// example.
	...
	yscore = (ION_MATCH_SCALE*ycount)/ionSet.yIons.length;		// 0 -25 for
																// example
	yscore +=(ION_INTENSITY_SCALE*Math.log10(yintensity)/ionSet.yIons.length);

	var consecResult = check_consecutive_ions(ionSet);		// the idea
															// being that
															// true
															// discoveries
															// will have
